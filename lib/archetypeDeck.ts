...
const LIVED_MOMENTS_FRAMING =
  " LIVED-MOMENT: caught mid-action, not posed — she is DOING the moment (mid-sip, mid-laugh, reaching, turning, glancing away), face clearly visible but she may look off-camera rather than straight into the lens. Compose for DEPTH using ONLY what the scene lock already contains — separate foreground / midground / background and shoot across or past the locked surfaces and objects so the space reads three-dimensional, not flat. Do NOT invent furniture, props or background elements the scene lock excludes; if the locked location is intentionally sparse, get depth from angle, distance and natural framing instead of adding things. Keep one real in-use prop only if the lock allows one. Slightly off-centre. Natural light, clean and well-exposed. Believable phone snapshot a friend took in a real place, not a studio portrait. Do NOT put more than one full face in frame.";
