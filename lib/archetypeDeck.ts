...
const LUXE_CAR_FRAMING =
  " LUXE-CAR FRAMING (mandatory — overrides any tighter crop): this is a NIGHT interior of a luxury car and the CABIN IS HALF THE SHOT. Frame a MEDIUM shot (roughly waist-up or wider — wide enough to hold a reclined, legs-drawn-up pose), NOT a tight face/chest crop — the quilted leather seat, the passenger window with streaking city / tunnel lights, and the warm ambient dash / footwell glow must all be clearly visible around her. She holds a DELIBERATE, body-forward passenger-princess pose that works as a hook on its own (reclined with knees drawn up or a heel on the seat, an arm along the seat back, leaning to the lens, a slow glance back) — never a passive upright sit. Low-key MOODY NIGHT lighting (warm amber cabin glow + one cool light streak across her), deep shadows, cinematic — never bright, flat, daylit or studio-sterile. She stays the focal point, but the luxury cockpit reads at a glance.";
