...
export type DoctrineKey = "cinematic" | "instagram" | "editorial" | "deepseek" | "nano_banana" | "caption";
...
// ── NANO BANANA doctrine ─────────────────────────────────────────
// Google Nano Banana safe prompts. iPhone 15 Pro lifestyle.
// Avoids IMAGE_SAFETY triggers: no "nude" as color, no detailed anatomy.
const NANO_BANANA_PHOTO_HEADER = `You are writing an image prompt for Google Nano Banana Pro (gemini-3.1-flash-image).

WRITE LIKE: a lifestyle photographer captioning an Instagram post. Short, specific, visual.

GOOGLE SAFETY RULES (non-negotiable):
- NEVER write "nude [garment]" — use "beige", "sand-colored", "ivory", "skin-tone" instead
- NEVER write body anatomy descriptions: no "smooth skin", "soft musculature", "narrow feminine shoulders", "bare back", "bare skin"
- NEVER write "suggestive", "sensual", "intimate", "erotic"
- Clothing must always be named specifically — never leave outfit unspecified

ABSOLUTE BANS: ethereal, dreamy, moody, cinematic grade, gorgeous, stunning, breathtaking, beautiful, perfect, flawless`;

const NANO_BANANA_PHOTO_OUTPUT = `OUTPUT RULES:
Write 50–80 words. Tag-based or short prose — whichever feels more natural for Instagram.

Cover in this order:
1. Shot type + angle (e.g. "Close-up portrait", "Medium shot from behind", "Waist-up at 3/4 angle")
2. Subject action (one verb — standing, looking, turning, leaning)
3. Outfit — use EXACTLY the wardrobe_lock from scene brief. NO substitutions. NO new items.
4. Setting — location from scene brief, 1-2 concrete details
5. Lighting — one source + direction
...
const NANO_BANANA_VIDEO_HEADER = `You are writing a video prompt for Google Veo 3.1 (veo-3.1-fast-generate-preview).

WRITE LIKE: a director's one-paragraph shot note. Plain motion vocabulary.

GOOGLE SAFETY RULES:
- No "nude [garment]" — use "beige", "sand-colored", "ivory"
- No body anatomy descriptions — describe clothing and movement, not skin
- No "sensual", "intimate", "suggestive"`;

const NANO_BANANA_VIDEO_OUTPUT = `OUTPUT RULES:
Write 50–80 words. Single paragraph.

Cover: camera movement, subject action (one mid-motion verb), setting, lighting, one physical element in motion (hair, fabric, shadow), loop logic.
Tech: "Duration: 7s, 24fps, 9:16"

End: "Signature: [palette] / [lens] / [motion]."
OUTPUT: Start with "Model: Seedance 2.0 🎬 Video Prompt" then the prompt.`;

// ── CAPTION doctrine ─────────────────────────────────────────────
// Ultra-short direct captions (20–45 words). Proven best for Google Nano Banana.
// No cinematic theory, no signatures required, no Frame contains blocks.
const CAPTION_PHOTO_HEADER = `You are writing a short image caption for Google Nano Banana Pro.

WRITE LIKE: a photographer's shot note — factual, direct, no theory.
LENGTH: 2–4 sentences. 20–45 words maximum.

GOOGLE SAFETY RULES:
- NEVER "nude [garment]" → use "beige", "sand-colored", "ivory"
- NO anatomical body descriptions ("smooth skin", "soft musculature", "bare back")
- Clothing must be named specifically`;
...
const CAPTION_VIDEO_HEADER = `You are writing a short video prompt for Google Veo 3.1 Fast.
...
OUTPUT: Start with "Model: Seedance 2.0 🎬 Video Prompt" then the caption.`;

const DOCTRINES: Record<DoctrineKey, DoctrineSpec> = {
  nano_banana: {
    photoStyleHeader: NANO_BANANA_PHOTO_HEADER,
    photoOutputRules: NANO_BANANA_PHOTO_OUTPUT,
    videoStyleHeader: NANO_BANANA_VIDEO_HEADER,
    videoOutputRules: NANO_BANANA_VIDEO_OUTPUT,
    photoMaxTokens: 300,
    videoMaxTokens: 300,
  },
  caption: {
    photoStyleHeader: CAPTION_PHOTO_HEADER,
...
// Minimal context for caption/nano_banana doctrines.
// Full commonBody() (400+ words of cinematic prose) overwhelms short-prompt
// instructions — Claude ignores "40 words max" when given a 400-word context.
// Caption needs only: wardrobe, slot, location, lighting.
function captionBody(args: BuildArgs): string {
  const archetypeAllowsProp =
    args.archetypeId === "hands_object" ||
    args.archetypeId === "interaction_object" ||
    args.archetypeId === "creator_process" ||
  ...
  const propLine = archetypeAllowsProp && (args.sceneBriefJson.allowed_props ?? []).length > 0
...
function buildPhotoSystem(args: BuildArgs): string {
  ...
  const body = (args.doctrine === "caption" || args.doctrine === "nano_banana")
    ? captionBody(args)
    : commonBody(args);
