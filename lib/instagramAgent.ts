...
- risk_level must be low, medium or high. reply must be null when should_reply is false.`,
    ...
    const { error } = await supabase.from("chs_ig_agent_runs").insert({
    ...
    if (error) throw new Error(`Unable to store Instagram agent run: ${error.message}`);
  } catch (error) {
    ...
    console.error("[instagram-agent] generation failed", message);
  }
}
