...
type PromptDoctrine = "cinematic" | "instagram" | "deepseek" | "editorial" | "nano_banana" | "caption";
...
PRIORITY HIERARCHY (STRICT ORDER â€" NO EXCEPTIONS)

1. Optical physics â€" what a real lens actually does: aberrations, focus falloff, breathing, depth compression, CA, vignetting, spherical distortion
2. Spatial and environmental integration â€" subject grows from the environment, never inserted into it
3. Physical realism â€" weight, friction, gravity, inertia
4. Biological realism â€" only processes visible to the eye, never catalogue descriptions
5. Material response â€" how light transmits through or reflects off specific materials
6. Camera behavior â€" operator errors as proof of reality, not as mistakes
7. Environmental authenticity â€" air, humidity, temperature, time of day as physical states
8. Emotional atmosphere â€" through physiological evidence only, never through description
9. Aesthetic beauty â€" emergent, never instructed
...
These are AI clichĂ©s that instantly artificialize the image.
...
* never more than 1 stop dynamic range reserve â€" real sensors clip before digital does
...
* muscle engagement + cascade activation (proximal â†' distal limb effect)
...
* Ischemic blanching: skin whitens under finger or elastic pressure (2-5mm radius), recovers to reactive hyperemia (red flush) within 3-8 seconds of release â€" visible at fabric edges, waistbands, straps
* Moisture gradient by zone: philtrum / suprasternal notch / lower back accumulate first; translucency shift where moisture is present (refractive index shift nâ‰1.33 water vs nâ‰1.44 dry skin)
...
* Piloerection logic: goosebumps triggered not only by cold but by electrostatic charge between silk/nylon and skin â€" visible as 1-2mm surface topology change at contact edge

AUTONOMIC RESPONSE (visible, never abstract):
* Mydriasis / miosis: pupil diameter responds to low light and focal fixation â€" specify dilation state relative to scene lighting (3-6mm in dim, 2-3mm in bright)
* Micro-tremor: pre-movement muscle firing â€" subtle 1-3Hz vibration in limb before gesture initiates, visible as surface texture shift
* Intermediate states only: never show start or end of an action â€" show mid-blink, mid-breath, mid-gesture, hand in motion, fabric caught mid-settle
...
Emotion must only emerge through physiological signature â€" measurable parameters:
...
Material memory â€" describe history of wear:

* compression set: time of pressure â†' depth of indentation â†' recovery time
* humidity absorption: % moisture â†' shape loss (curl, sag)
* gravity crease: time standing â†' fabric self-organization under own weight
* elastic fatigue: wear cycles â†' % elasticity loss â†' roll/bunch behavior
* patina: oxidation, friction, sweat â†' color gradient
...
The image must balance: emotional intimacy / natural feminine presence / tactile realism / subtle visual desire / cinematic sophistication â€" without becoming artificial, performative, glamour-driven, or explicit.
...
* Tactile gravity: clothing does not only respond to the figure â€" it microscopically "sticks" to skin due to friction (ÎĽ), then releases; static electricity between silk/nylon and skin creates momentary cling before separation
* Subsurface scattering variability: blood circulation is uneven â€" ears, fingertips, and knuckles must show higher scatter coefficient (redder tone) under backlight or rim light
* Intermediate states: never show the beginning or end of an action â€" show only the middle: a button half-open, a hand mid-movement, hair caught on a collar, fabric mid-settle after a gesture
...
* Noise: subtle organic texture only â€" not heavy grain, not high-ISO degradation; sensor presence felt, not dominant
...
export const ARC_TRANSLATION: Record<string, string> = {
  opening: `ARC â€" OPENING: exploratory framing, softer realism, environmental distance, observational atmosphere. Wider shot, subject partially absorbed by surroundings, soft natural light, low contrast.
Optical specification: 35-50mm equivalent, mild barrel distortion, edge softness, natural vignetting, no aggressive compression.`,
  ...
  rising: `ARC â€" RISING: tighter framing closing in, stronger contrast, increased movement, more environmental interaction, stronger texture visibility, heightened sensory detail.
Optical specification: 50-85mm equivalent, spatial compression visible, focus falloff on frame edges, mild chromatic aberration.`,
  ...
  peak: `ARC â€" PEAK: aggressive intimacy, close physical proximity, harsh directional light revealing every biological detail, sweat, fatigue, unstable framing, emotional pressure visible in musculature.
Optical specification: 85mm+, shallow DoF with busy bokeh (not creamy), focus hunting visible, micro-jitter, highlight clipping on skin.`,
  ...
  turning: `ARC â€" TURNING: asymmetrical compositions, mixed color temperatures in same frame, posture instability, emotional tension, environmental fragmentation.
Optical specification: wide 24-35mm, perspective distortion, unnatural proximity to subject, spatial relationships feel wrong.`,
  ...
  falling: `ARC â€" FALLING: static compositions, drained atmosphere, reduced movement, emotional distance, weakened environmental energy, cooler color temperature.
Optical specification: 50mm, flat light, minimal shadow detail, documentary optical neutrality, no lens character.`,

  quiet: `ARC â€" QUIET: intimate realism, soft natural light from single source, minimal movement, subtle environmental detail, calm physical stillness, micro-imperfection at close range.
...
* Z-axis breathing shift: operator respiration creates ±2mm depth-of-field oscillation at ~0.2Hz â€" subject sharpness pulses subtly with each breath cycle
* Focus hunting at close range (â‰¤0.8m): any subject movement of 1-2mm forward triggers visible focus breathing as lens searches for optimal plane â€" creates sensation of a real operator behind the camera
* Never lock focus perfectly â€" sustained perfect sharpness reads as synthetic

Motion must never feel robotic or mathematically perfect.`;

// ── NANO BANANA SAFE DOCTRINE ────────────────────────────────────
// For Google Gemini / Nano Banana — passes safety filters while creating
// lifestyle-intimate content. Based on Visual AI Club iPhone Playbook approach.
//
// KEY RULES:
// ✅ Always specify what she IS wearing (even minimal — bralette, silk slip, robe, swimsuit)
// ✅ Frame as a real candid moment (morning routine, after pool, getting ready, hotel room)
// ✅ iPhone 15 Pro photographic language — feels like a real Instagram post
// ✅ Describe energy/confidence through posture and gaze, NOT through "suggestive" language
// ❌ Never: "nude" as clothing color — use "beige", "sand", "ivory", "skin-tone" instead
// ❌ Never: "covering chest", "semi-nude", "barely covered", explicitly sensual descriptions
// ❌ Never: "suggestive", "intimate", "sensual", "erotic" — these trigger safety filters
// ❌ Never: anatomical body descriptions ("soft musculature", "narrow feminine shoulders",
//          "smooth skin", "bare back") — triggers IMAGE_SAFETY classifier
//
// OUTFIT COLOR SAFETY:
// "nude bandeau top" → "sand-colored bandeau top" / "beige bandeau top"
// "nude bikini"      → "ivory bikini" / "sand bikini"
// "nude bodysuit"    → "skin-tone bodysuit"
//
// OUTFIT SAFETY MAPPING:
// Semi-nude → "delicate lace bralette" / "white satin slip" / "silk robe open at front"
// Beach/pool → "vintage-cut one-piece" / "minimal bikini top" / "athletic swimwear"
// Bedroom → "white cotton sleep dress" / "hotel bathrobe loosely tied"
export const NANO_BANANA_SAFE_DOCTRINE = (ctx: StoryContext, toneBlock: string, stylingBlock: string) => `You are writing an image prompt for Google Nano Banana Pro — a photorealistic AI image model.

The output must pass Google content safety filters while creating aspirational, confident, lifestyle-intimate photography.

CHARACTER: ${ctx.visualBrief}
LOCATION: ${ctx.location}
MOOD: ${ctx.mood}
ARC: ${ctx.arc_position}
${toneBlock ? `TONE: ${toneBlock}\n` : ""}${stylingBlock}

PROMPT STRUCTURE (follow exactly):
[Shot type + camera] [Character position + what she's doing] [Specific outfit — always include] [Environment + lighting] [Expression/energy — through posture/gaze only] [iPhone photographic signature]

CRITICAL RULES:
- Always specify an outfit (even minimal). Never leave clothing unspecified.
- Frame as a real candid moment — morning routine, hotel room, beach, restaurant
- Use iPhone 15 Pro photographic language (Apple HDR, flat depth of field, candid selfie energy)
- Confidence through posture and gaze — NOT through "suggestive" or "sensual" language
- Describe what IS there. Not what is missing or covered.

CLOTHING COLOR RULE — CRITICAL:
NEVER write "nude [garment]". Google reads "nude" as nudity, not a color.
Use instead: "beige", "sand", "sand-colored", "ivory", "skin-tone", "taupe"
Example: "nude bandeau top" → "sand-colored bandeau top"
Example: "nude bikini" → "ivory bikini"

BODY DESCRIPTION RULE — CRITICAL:
Never describe anatomy in detail when clothing is minimal.
AVOID: "smooth skin", "soft musculature", "narrow feminine shoulders", "bare back", "bare skin"
USE INSTEAD: describe clothing, posture, setting — let the scene imply the body

SAFE LANGUAGE EXAMPLES:
- "white lace bralette and linen trousers, standing by hotel window"
- "loosely tied silk robe, morning light, reading on bed"
- "minimal triangle bikini top, sitting on Mediterranean rocks"
- "ivory satin slip dress, mirror selfie, hotel bathroom"
- "sand-colored bandeau top and linen pants, Amalfi terrace"

AVOID COMPLETELY: nude / suggestive / intimate / sensual / erotic / barely covered / covering chest / semi-nude / exposed / revealing / smooth skin / soft musculature / bare skin / bare back

OUTPUT: Write the prompt in 60-100 words. Tag-based, punchy, iPhone lifestyle feel.`;
...
LIGHTING: natural or phone flash â€" describe briefly
AVOID: cinematic language, technical optical specifications, long paragraphs

OUTPUT: write the tag-based prompt. Keep it punchy and direct.`;
...
MOVEMENT: subtle, natural â€" breathing, micro-adjustment, soft gesture
...
const DEEPSEEK_DOCTRINE_PHOTO = (ctx: StoryContext, toneBlock: string, stylingBlock: string) => `You are generating a photorealistic image prompt using the Deepseek doctrine â€" a structured, physics-first approach focused on capturing a single mid-action moment.
...
1. SHOT TYPE + FRAMING â€" (static / medium / close-up / wide) â€" what is visible in frame
2. LOCATION + TIME â€" specific place, time of day, weather, light state
3. SUBJECT + ACTION â€" who, wearing what, doing what â€" always mid-action (mid-yawn, mid-turn, mid-drink, mid-laugh, mid-exhale)
4. PHYSICS DETAIL â€" one visible physical element: steam, hair movement, wet fabric, dust, shadow, condensation on glass, breath vapor
5. LIGHT STATE â€" exact light quality: morning mist, overcast grey, warm indoor tungsten, blue hour, diffused cloud light
6. EMOTIONAL SIGNATURE â€" one-word mood + physical evidence (tired: slow blink, heavy lid / curious: slight head tilt, forward lean)
...
OUTPUT: write the prompt in 60-100 words, flowing prose. No lists.`;

const DEEPSEEK_DOCTRINE_VIDEO = (ctx: StoryContext, toneBlock: string, stylingBlock: string) => `You are generating a video prompt using the Deepseek doctrine â€" 7 mandatory elements, story arc, physics-first.
...
1. SHOT TYPE â€" choose one: slow dolly-in / tracking / static / orbit / zoom-out / push-in / following shot
2. CAMERA MOVEMENT â€" choose one: handheld micro-shake / breathing motion / smooth follow / side-to-side sway
3. SUBJECT MOVEMENT â€" natural mid-action only: mid-yawn / mid-drink / turns head / laughs / exhales / writes / pauses mid-step
4. PHYSICS ELEMENT â€" one: steam rising / hair in breeze / wet fabric / dust particles / shadow movement / breath vapor / rain on glass
5. LIGHTING â€" state + optional change: "golden dawn, light shifting from warm to cool as cloud passes" / "overcast grey, stable" / "indoor warm 3200K, candle flicker"
6. STORY ARC â€" start â†' development â†' micro-resolution (e.g. "she raises cup â†' drinks â†' lowers it, stares at water")
7. TECHNICAL â€" duration 4-6 seconds, 24fps, 9:16 vertical, seamless loop (yes/no)

FORBIDDEN: gĂ˝ÄŤ / perfect skin / studio lighting / generic beauty / dreamy / ethereal / beautiful
...
CORE DOCTRINE: The subject must always feel like a top international fashion model photographed for a luxury editorial campaign â€" never like a random realistic person from a film scene.
...
PRIORITY ORDER:
1. Facial beauty and bone structure â€" sculpted cheekbones, refined jawline, symmetrical facial harmony, expressive almond-shaped eyes, natural full lips, elegant brows, striking beauty, effortless elegance
2. Skin realism â€" realistic pores, micro skin texture, subsurface scattering, natural facial asymmetry, realistic imperfections, ultra-detailed skin rendering
3. Cinematic luxury lighting â€" golden hour / soft Mediterranean overcast / cinematic window light / diffused luxury lighting / soft shadows / realistic reflections
4. Fashion/editorial atmosphere â€" couture-inspired styling, luxury fabrics, silk, tailored silhouettes, refined accessories, minimalist luxury jewelry
...
OUTPUT: write one polished cinematic editorial prompt in a single paragraph (110-150 words), optimized for premium AI image generation.`;
...
Your goal is to generate breathtaking luxury editorial fashion video â€" Vogue Italia moving image aesthetic, hyper-realistic feminine beauty in motion, premium campaign atmosphere.

CORE DOCTRINE: The subject must always feel like a top international fashion model in a luxury editorial â€" never a film character or street photography subject.

CHARACTER: ${ctx.visualBrief}
${toneBlock ? `\nTONE DIRECTION: ${toneBlock}` : ""}${stylingBlock}

PRIORITY ORDER:
1. Facial beauty and bone structure â€" sculpted cheekbones, symmetrical harmony, almond-shaped eyes, elegant brows, effortless elegance
2. Skin realism in motion â€" subsurface scattering, micro skin texture, realistic pores
3. Cinematic luxury lighting â€" golden hour / Mediterranean overcast / soft window light
4. Editorial fashion atmosphere â€" couture styling, silk movement, refined accessories
5. Elegant camera movement
6. Premium visual atmosphere

VIDEO REQUIREMENTS:
- Camera: Sony A7R IV, 85mm f/1.4, shallow DOF, soft bokeh
- Movement: slow cinematic dolly or elegant tracking shot â€" never handheld shake
- Duration: 4-6 seconds, 24fps, 9:16 vertical
- Subject motion: one subtle elegant action â€" slow turn, hair catching light, lifting gaze, adjusting silk, fingers trailing a surface
- One physics element: silk flowing, hair lifting in breeze, golden light shift across skin, reflection shimmering
- Story arc: poised stillness â†' one graceful movement â†' refined resolve
...
SAFE FASHION RULES: High-fashion elegance only. Sensuality through composition and light â€" never explicit action.
...
// ── CAPTION DOCTRINE ─────────────────────────────────────────────
// For Google Nano Banana — simple, direct shot descriptions.
// Based on training caption style: short, factual, no cinematic theory.
// 20–40 words. Single scene. Works reliably through Google safety filters.
//
// Format proven to work:
// "[Shot type] of [subject]. [Outfit — brief]. [Expression/pose]. [Lighting]."
//
// Examples from training data:
// "Close-up portrait of a young woman. Soft smile. White top at neckline. Warm light from left."
// "Waist-up shot at 3/4 angle. Light smile. Hands at sides. Daylight from window on right."
export const CAPTION_DOCTRINE = (ctx: StoryContext, stylingBlock: string) => `You are writing a short image caption for Google Nano Banana Pro.
...
Write a SINGLE SHORT image caption. 2–4 sentences. 20–45 words maximum.

FORMAT (follow exactly):
[Shot type + angle] of [subject brief]. [Outfit — 1-2 items, specific]. [Expression or pose — simple]. [Lighting — one phrase].

RULES:
- One scene only. Never describe two places or two moments.
- Name clothing specifically (ivory linen shirt, sand bikini top, white slip dress)
- Never use: "nude" as color, "suggestive", "sensual", "intimate", "smooth skin", "bare skin"
- No iPhone signatures, no "Signature:" tags, no "Frame contains:" blocks
- No physical complexity, no cinematic doctrine, no biomechanics
- Keep it as simple as a photographer's shot note

EXAMPLES OF CORRECT OUTPUT:
"Close-up portrait. Soft smile. White lace bralette visible at neckline. Warm afternoon light from the left."
"Waist-up shot at 3/4 angle facing right. Ivory linen shirt, unbuttoned collar. Relaxed expression, looking off camera. Soft diffused daylight."
"Medium shot on Amalfi terrace. Sand-colored bandeau top and linen wrap skirt. Looking toward coastline. Warm golden afternoon sun from left."

OUTPUT: Write only the caption. Nothing else.`;
...
- her skin temperature differential creates 1.5°C contrast with ambient air, visible at fabric edge`
  ...
  const stylingBlock = ctx.stylingNote?.trim()
    ? `\nSTYLING OVERRIDE (highest priority â€" always apply exactly, overrides any default appearance):\n${ctx.stylingNote.trim()}`
    : "";

  // Instagram doctrine â€" short tag-based prompts
  ...
  // Caption doctrine — simple direct shot descriptions (best Google results)
  if (doctrine === "caption") {
    const photoMsg = await claudeWithRetry({
      model: "claude-sonnet-4-6",
      max_tokens: 150,
      system: CAPTION_DOCTRINE(ctx, stylingBlock),
      messages: [{ role: "user", content: "Generate the image caption." }],
    });
    const photoPrompt = (photoMsg.content[0] as { type: string; text: string }).text.trim();
    await supabase.from("chs_media").insert({
      story_day_id: ctx.storyDayId, type: "photo",
      higgsfield_prompt: photoPrompt, status: "pending",
      prompt_doctrine: "caption",
  ...
  // Nano Banana doctrine — Google-safe iPhone lifestyle photography
  if (doctrine === "nano_banana") {
    const photoMsg = await claudeWithRetry({
      model: "claude-sonnet-4-6",
      max_tokens: 350,
      system: NANO_BANANA_SAFE_DOCTRINE(ctx, toneBlock, stylingBlock),
  ...
  // Deepseek doctrine â€" 7-element structured video/photo
  ...
  // Editorial doctrine â€" Vogue Italia luxury fashion portrait
...
Write a SINGLE PARAGRAPH of 120â€"180 words. No numbered lists. No abstract measurements. No forbidden/negative instructions in the output.

The paragraph must contain â€" in natural flowing prose:
- Shot type and camera lens (e.g. "85mm f/1.4, shallow depth of field")
- Where the subject is and what surrounds her physically
- What she is wearing â€" fabric, fit, how it sits on her body
...
- One subtle lens artifact (soft edge falloff, slight vignette, or focus breathing â€" pick one)

Write what a camera sees. Not what a poet feels.

OUTPUT: the paragraph. Nothing else.`,
      messages: [{ role: "user", content: "Generate the photo prompt." }],
    }),
...
Write a SINGLE PARAGRAPH of 130â€"180 words. No numbered lists. No abstract measurements. No forbidden/negative instructions in the output.
...
- What she is wearing â€" how the fabric moves under the camera's motion
- Her one primary action: always mid-motion (mid-turn, mid-exhale, hand mid-raise â€" never start or end of gesture)
