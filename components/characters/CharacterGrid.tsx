...
export default function CharacterGrid({ characters, activeCount, photoMap }: Props) {
  ...
  return (
    <>
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="mb-4 px-4 py-2.5 font-mono text-[11px] bg-red-900/10 border border-red-500/20 text-red-400"
          >
            {error}
          </motion.div>
        )}
      ...
      <motion.div
        className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-px bg-border mb-px"
      ...
      >
        ...
        <motion.a
          ...
          className="bg-surface border border-border border-dashed flex flex-col items-center justify-center gap-3 p-5 min-h-[260px]"
          whileHover={{ borderColor: "rgba(65,238,194,0.4)", backgroundColor: "rgba(17,19,25,1)" }}
          whileTap={{ scale: 0.98 }}
        >
          <motion.span
            className="material-symbols-outlined text-[28px] text-muted"
      ...
      <motion.div
        className="flex items-center gap-8 border border-border px-5 py-3 bg-surface"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
...
const DOCTRINE_NOTES: Record<string, string> = {
  caption: "Krátke, priame popisy scén (20–45 slov). Štýl fotografického záznamu — shot type, oblečenie, výraz, svetlo. Najlepšie výsledky cez Google Nano Banana. Bez cinematic teórie.",
  nano_banana: "Google Nano Banana safe prompts. iPhone 15 Pro candid lifestyle. Always specifies outfit. Passes safety filters — no sensual language, confidence through posture/gaze.",
